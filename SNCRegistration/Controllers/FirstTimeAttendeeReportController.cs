    ...
    public class FirstTimeAttendeeReportController: Controller
        ...
        public ActionResult GetFirstTimeAttendeeByYear(int eventYear)
            ...
            using (var connection = new SqlConnection(constring))
                ...
                dt = new DataTable();
                connection.Open();
                query = "SELECT ParticipantID, ParticipantFirstName, ParticipantLastName, CASE WHEN Returning = 1 THEN 'Yes' ELSE 'No' END AS Returning, Description FROM Participants INNER JOIN Attendance ON AttendingCode = AttendanceID WHERE Returning = 0 AND EventYear = @EventYear Order By Description";
                using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
                    {
        ...
        public ActionResult FirstTimeAttendeeReport(int eventYear)
            ...
            string query = "SELECT ParticipantID, ParticipantFirstName, ParticipantLastName, CASE WHEN Returning = 1 THEN 'Yes' ELSE 'No' END AS Returning, Description FROM Participants INNER JOIN Attendance ON AttendingCode = AttendanceID WHERE Returning = 0 AND EventYear = @EventYear Order By Description";
            DataTable dt = new DataTable();
